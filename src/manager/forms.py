...
class DefaultForm(forms.Form):
    ...
    title = forms.CharField(widget=forms.TextInput, required=True, label='Book Title')
...
class DefaultNotRequiredForm(forms.Form):
    ...
    title = forms.CharField(widget=forms.TextInput, required=False, label='Book Title')
    subtitle = forms.CharField(widget=forms.TextInput, required=False)
...
class GeneratedNotRequiredForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for relation in relations:
            ...
            if relation.element.field_type == 'text':
                self.fields[relation.element.name] = forms.CharField(widget=forms.TextInput(attrs={'div_class': relation.width}), required=False)
            elif relation.element.field_type == 'textarea':
                ...
                if relation.element.name == 'Series':
                    ...
                    choices = render_choices(relation.element.choices)
                self.fields[relation.element.name] = forms.ChoiceField(widget=forms.Select(attrs={'div_class': relation.width}), choices=choices, required=False)
            elif relation.element.field_type == 'email':
                self.fields[relation.element.name] = forms.EmailField(widget=forms.TextInput(attrs={'div_class': relation.width}), required=False)
            elif relation.element.field_type == 'check':
                self.fields[relation.element.name] = forms.BooleanField(widget=forms.CheckboxInput(attrs={'is_checkbox': True, 'div_class': relation.width}), required=False)
            self.fields[relation.element.name].help_text = relation.help_text
            self.fields[relation.element.name].label = relation.element.name
